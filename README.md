...
複数のAPIキーを登録して制限を分散する、1分で処理できる制限を超えそうになると待機してエラーを回避する。

API キーは **プロジェクトごとに1本**を目安に発行してください。レート制限（15 req/min）はプロジェクト単位で適用されるため、同じプロジェクト内でキーを増やしても意味がありません。Google AI Studio で複数のプロジェクトを作成し、それぞれからキーを取得するのがおすすめです。
